...
**三层可观测性体系**，将 Agent 全生命周期抽象为 23 种类型化事件：

| 层 | 机制 | 用途 |
|----|------|------|
| EventBus 实时推送 | `IpcEventBroadcaster` → TCP → 客户端 | 前端实时渲染 |
| events.jsonl 落盘 | `EventWriter` → runs/\<id\>/events.jsonl | 历史回放 |
| Trace 全量记录 | `TraceWriter` → daemon.jsonl | 开发者调试 |

`TracingProvider` 装饰 LLMProvider，在每次 `chat()` 调用前后自动记录完整的请求/响应 payload + 延迟（ms）。`IpcEventBroadcaster` 支持 fnmatch topic 过滤（如 `llm.*`）和 scope 过滤（`global` / `run:<id>`），客户端按需订阅。
