...
_DEFAULT_LOG_FILE = "~/.wuwu/logs/core.log"
_DEFAULT_LOG_FORMAT = "text"
_DEFAULT_CONFIG_PATH = "~/.wuwu/config.toml"
_DEFAULT_MAX_STEPS = 20
_DEFAULT_MODEL = "deepseek-v4-flash"
_DEFAULT_TRACE_FILE = "~/.wuwu/traces/daemon.jsonl"
...
def get_config() -> CodeConfig:
    config = CodeConfig()
    # .env 必须在读取 CODE_CONFIG 之前加载，以便 .env 中的 CODE_CONFIG 能影响 TOML 路径
    load_dotenv(".env", override=False)

    # 若显式执行 toml文件，则只读该文件
    explicit = os.environ.get("CODE_CONFIG")
...
def _apply_toml(config: CodeConfig, data: dict[str, Any]) -> None:
    ...
    if "permission" in data:
        ...
        if unknown_perm:
            raise SystemExit(
                f"Unknown [permission] keys: {', '.join(sorted(unknown_perm))}"
            )
...
# 用 CODE_* 环境变量覆盖 config 中对应字段（若变量已设置）
def _apply_env(config: CodeConfig) -> None:
    host = os.environ.get("CODE_HOST")
    if host is not None:
        config.host = host

    port_str = os.environ.get("CODE_PORT")
    if port_str is not None:
        try:
            config.port = int(port_str)
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_PORT must be an integer, got: {port_str!r}"
            )

    log_level = os.environ.get("CODE_LOG_LEVEL")
    if log_level is not None:
        config.logging.level = log_level

    log_file = os.environ.get("CODE_LOG_FILE")
    if log_file is not None:
        config.logging.file = log_file

    log_format = os.environ.get("CODE_LOG_FORMAT")
    if log_format is not None:
        config.logging.format = log_format

    max_steps_str = os.environ.get("CODE_MAX_STEPS")
    if max_steps_str is not None:
        try:
            val = int(max_steps_str)
            if val <= 0:
                raise SystemExit(
                    "Config error: CODE_MAX_STEPS must be a positive integer,"
                    f" got: {max_steps_str!r}"
                )
            config.agent.max_steps = val
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_MAX_STEPS must be an integer, got: {max_steps_str!r}"
            )

    default_model = os.environ.get("CODE_LLM_DEFAULT_MODEL")
    if default_model is not None:
        config.llm.default_model = default_model

    trace_enabled = os.environ.get("CODE_TRACE_ENABLED")
    if trace_enabled is not None:
        config.trace.enabled = trace_enabled.lower() not in ("0", "false", "no")

    trace_file = os.environ.get("CODE_TRACE_FILE")
    if trace_file is not None:
        config.trace.file = trace_file

    trace_payload = os.environ.get("CODE_TRACE_INCLUDE_LLM_PAYLOAD")
    ...
    perm_timeout = os.environ.get("CODE_PERMISSION_TIMEOUT_S")
    if perm_timeout is not None:
        try:
            perm_timeout_val = float(perm_timeout)
            if perm_timeout_val < 0:
                raise SystemExit(
                    f"Config error: CODE_PERMISSION_TIMEOUT_S must be >= 0, got: {perm_timeout!r}"
                )
            config.permission.timeout_s = perm_timeout_val
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_PERMISSION_TIMEOUT_S must be a number, got: {perm_timeout!r}"
            )

    compact_threshold = os.environ.get("CODE_COMPACT_THRESHOLD")
    if compact_threshold is not None:
        try:
            compact_threshold_val = float(compact_threshold)
            if not (0.0 <= compact_threshold_val <= 1.0):
                raise SystemExit(
                    f"Config error: CODE_COMPACT_THRESHOLD must be between 0 and 1, got: {compact_threshold!r}"
                )
            config.compaction.auto_threshold = compact_threshold_val
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_COMPACT_THRESHOLD must be a number, got: {compact_threshold!r}"
            )

    compact_tool_limit = os.environ.get("CODE_COMPACT_TOOL_LIMIT")
    if compact_tool_limit is not None:
        try:
            compact_tool_limit_val = int(compact_tool_limit)
            if compact_tool_limit_val <= 0:
                raise SystemExit(
                    f"Config error: CODE_COMPACT_TOOL_LIMIT must be a positive integer, got: {compact_tool_limit!r}"
                )
            config.compaction.tool_result_limit = compact_tool_limit_val
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_COMPACT_TOOL_LIMIT must be an integer, got: {compact_tool_limit!r}"
            )

    compact_tool_keep = os.environ.get("CODE_COMPACT_TOOL_KEEP")
    if compact_tool_keep is not None:
        try:
            compact_tool_keep_val = int(compact_tool_keep)
            if compact_tool_keep_val <= 0:
                raise SystemExit(
                    f"Config error: CODE_COMPACT_TOOL_KEEP must be a positive integer, got: {compact_tool_keep!r}"
                )
            config.compaction.tool_result_keep = compact_tool_keep_val
        except ValueError:
            raise SystemExit(
                f"Config error: CODE_COMPACT_TOOL_KEEP must be an integer, got: {compact_tool_keep!r}"
            )
