...
# 每个连接处理协程中，当前正在处理的writer(供 handler读取链接上下文)
...
class SocketServer:
    ...
    async def _handle_connection(
        ...
        peer = writer.get_extra_info("peername", "<unknow>")
    ...
    async def _read_loop(
        ...
        while True:
            try:
                ...
                await self._send(
                    writer, make_error(None, INVALID_REQUEST, "Request too large")
                )
                return
    ...
    async def _handle_line(self, line: bytes, writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            await self._send(
                writer, make_error(None, INVALID_REQUEST, "Invalid Request", str(e))
            )
            return
        ...
        try:
            ...
            await self._send(
                writer, make_error(req.id, INTERNAL_ERROR, "Internal error")
            )
            return
